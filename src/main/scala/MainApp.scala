...
object MainApp {
  ...
  def filterMenu(records: List[EnergyRecord]): Unit = {
    ...
    println("6. Filter by week")
    println("7. Filter by plant status")
    ...
    choice match {
      ...
      case "6" =>
        println("Enter the start date of the week:")
        val startDate = readValidDate()
        println(s"\nWeekly records from $startDate (7-day window):")
        Utils.printRecords(QueryService.filterByWeek(records, startDate))
        filterMenu(records)
    ...
    choice match {
      ...
      case "4" =>
        ...
        if (showHelp == "y") {
          ...
          println("- Plant status: Normal, LowOutput, MaintenanceNeeded, Malfunction")
      ...
      case "6" =>
        ...
        val summary = AlertService.countBySeverity(alerts)
        println("\nAlert summary:")
        println(s"Critical: ${summary.getOrElse(Critical, 0)}")
        println(s"Warning: ${summary.getOrElse(Warning, 0)}")
        println(s"Info: ${summary.getOrElse(Info, 0)}")
